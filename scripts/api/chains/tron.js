...
const TronWebGlobal = typeof window !== 'undefined' && window.TronWeb
    ? window.TronWeb
    : null;
if (!TronWebGlobal) {
...
function normalizeTronResponse(data, txId) {
    if (!data || !data.hash) {
    ...
    let transactionType = 'NATIVE_TRANSFER'; // Default
    let tokenDetails = undefined;
    ...
    // Priority 2: Contract Interactions (like transfer, approve) often found in trigger_info for TRC20
        ...
        if (tokenInfo) { // Must have tokenInfo to be a token transfer
            ...
            let rawAmountFromParam = undefined;
...
export async function fetchTronTx(txId) {
    ...
    try {
        return await cached(`tron:tx:${txId}`, async () => {
            const rawData = await safeJsonFetch(url, { headers });
...
export async function fetchTronAddr(address) {
    ...
    try {
        ...
        const transferListData = await cached(`tron:addr-latest-trc20:${address}`, () => safeJsonFetch(url, { headers }));
