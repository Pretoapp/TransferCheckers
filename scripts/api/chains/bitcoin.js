...
function normalizeBitcoinResponse(data, txHash) {
    if (!data || data.error) {
        return {
            status: 'failed',
    ...
    const fromAddresses = data.inputs?.map((input) => ({
    ...
    const toAddresses = data.outputs?.map((output) => ({
...
export async function fetchBitcoinTx(txHash) {
    ...
    try {
        return await cached(`bitcoin:tx:${txHash}`, async () => {
            const rawData = await safeJsonFetch(url); // Specify expected raw type from Blockcypher
