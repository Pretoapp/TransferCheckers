...
function normalizeEthResponse(txData, receiptStatus, txHash) {
...
export async function fetchEvmTx(txHash, chainName /* | 'polygon' | ... */ // Chain name for targeting correct API if this function handles multiple EVM chains
) {
    if (!ETHERSCAN_KEY) {
    ...
    try {
        ...
        return await cached(`evm:<span class="math-inline">\{chainName\}\:</span>{txHash}`, async () => {
            ...
            const receiptResJson = await safeJsonFetch(receiptURL); // Specify expected raw type
            ...
            const txResJson = await safeJsonFetch(txURL); // Specify expected raw type
